...
# load csv
csv_files = list(Path("/home/zx/code/nba_api-master/assets/fromq3").glob("*.csv"))
logger.info(f"len of csv_files: {len(csv_files)}")
...
def get_team_token(game_date: str, tag_slug) -> dict:
    ...
    for event in filtered_events:
        for market in event["markets"]:
            ...
            outcomes = market["outcomes"].replace("[", "").replace("]", "").replace("\"", "").split(", ")
            clobTokenIds = market["clobTokenIds"].replace("[", "").replace("]", "").replace("\"", "").split(", ")
            for outcome, clob_token_id in zip(outcomes, clobTokenIds):
                assert outcome not in outcome_tokens, f"outcome: {outcome} already exists"
...
def check_flip(time_played, score_diff):
    ...
    if len(data_over_score_diff) < 100:
        logger.info(f"only {len(data_over_score_diff)} games, not enough to check flip rate")
        return 100
    ...
    data_over_score_diff["product"]= data_over_score_diff.apply(lambda row: calculate_row_product(row, time_played), axis=1)
    fliped_games = data_over_score_diff[data_over_score_diff["product"] < 0]
    # assert len(fliped_games) + len(unfliped_games) == len(data_over_score_diff), "error"
